...
function mapImagesToPexel(images) {
  return images.map((image) => {
    return {
      id: image.id,
      src: `${
        image.image ? image.image.url : "/uploads/default_image_943043ab9f.png"
      }`,
      title: image.title,
...
export const initImages = () => {
  return (dispatch) => {
    axios.get("/images?_sort=id:DESC&_limit=50").then((response) => {
      if (!response) {
        return;
      }
...
export const searchImages = (queryText) => {
  return (dispatch) => {
    ...
    axios.get(`/images?title_contains=` + queryText).then((response) => {
      if (!response) {
        return;
      }
